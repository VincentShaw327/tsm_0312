...
// import CTextItem from '../topBCommon/CreateTextItem';
  ...
  let tableFeature = React.createClass( {
    ...
    getInitialState: function () {
      return {
        ...
        total: 0,
        pageSize: 10,
        size: config.size ? config.size : 'middle',
    ...
    handlePlace: function ( info ) {
      ...
      const self = this;
      let result = Immutable.fromJS( self.state.resultList );
      let infoN = Immutable.fromJS( self.state.updateFromItem )
        .merge( info )
        .toJS();
    ...
    render: function () {
      ...
      if ( config.pageData ) {
        ...
        table = <Table rowSelection={this.state.isSelection?rowSelection:null} dataSource={this.state.resultList} columns={this.state.columns} loading={this.state.loading} pagination={pagination} size={this.state.size} />;
      } else {
        table = <Table rowSelection={this.state.isSelection?rowSelection:null} dataSource={this.state.resultList} columns={this.state.columns} loading={this.state.loading} size={this.state.size} />;
      }
      ...
      return <div className={this.props.className} >
              {/* {Slider} */}
                <div style={{ display: 'flex', alignItems: 'center', position: 'relative', margin: '0.5% 0% 10px', borderBottom: '1.6px solid #e8e8e8', padding: '6px 0 6px' }}>
                    <span style={{ flexGrow: '1' }}><Tiltle typeIcon={'table'} centent={'查询列表'}/></span>
                  ...
                  {/* <PForm UType={config.PType}  submit={self.handlePlace}  isShow={this.state.placeFromShow} updateItem={this.state.updateFromItem} hideForm={this.hideUpdateForm}/> */}
                </div>
                {table}
